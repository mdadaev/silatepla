...
class Prototype
{
	...
	*/
	 ...
	 * $aMenuLinks = array_merge($aMenuLinks, \Esta\Main\Iblock\Prototype::getInstance('Content-News')->getElementsForMenu());
	 * 
	...
	/**
	 * Добавляет обработчик картинок для полей элемента инфоблока
	 *
	 * @param Main\Image\Handler $handler
	 * @return void
	 */
	public function addImageHandler(Main\Image\Handler $handler)
	{
		if (!$handler->originalIblockField) {
			throw new Main\Exception('Field code is not defined');
		}
		
		//TODO: performance optimization with use OnBeforeIBlockElementAdd|OnBeforeIBlockElementUpdate and direct $arFileds modification
		if (!$this->imageHandlers) {
			$eventManager = \Bitrix\Main\EventManager::getInstance();
			$eventManager->addEventHandler(
				'iblock',
				'OnAfterIBlockElementAdd',
				array($this, 'onAfterIBlockElementAddUpdate')
			);
			$eventManager->addEventHandler(
				'iblock',
		...
		$this->imageHandlers[$handler->originalIblockField] = $handler;
	}
	...
	/**
	 * Определяет константы вида Esta\Main\Iblock\ID_{CODE} и Esta\Main\Iblock\CODE_{ID} для всех инфоблоков
	 *
	 * @param integer $cacheTime Время кэширования
	 * @return void
	 */
