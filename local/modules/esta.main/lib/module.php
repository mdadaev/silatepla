...
class Module
{
	...
	protected static function checkUnavailableFunctions()
	{
		if (!function_exists('json_encode')) {
			include_once \Esta\Main\BASE_DIR . '/functions/json_encode.php';
		}
	}
	...
	protected static function setupEventHandlers()
	{
		...
		//$eventManager->addEventHandler('form', 'onAfterResultAdd', array('\Esta\Main\Form', 'onAfterResultAdd'));
		
		/* Sale event handlers */
		//$eventManager->addEventHandler('sale', 'OnBeforeBasketAdd', array('\Esta\Main\Sale\Basket', 'onBeforeBasketAdd'));
		//$eventManager->addEventHandler('sale', 'OnBasketAdd', array('\Esta\Main\Sale\Basket', 'onBasketAdd'));
		//$eventManager->addEventHandler('sale', 'OnOrderUpdate', array('\Esta\Main\Sale\Order', 'onOrderUpdate'));
		$eventManager->addEventHandler('main', 'OnBeforeEventAdd', array('\Esta\Main\Form', 'OnBeforeEventAddHandler'));
	}
}