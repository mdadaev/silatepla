...
if ($_POST["submit-form"]) {
    ...
    //если нет ошибок
        ...
        if ($PRODUCT_ID = $el->Add($arLoadProductArray)) {
            ...
            $iblockCode = Main\Iblock\Prototype::getCodeById($arParams["IBLOCK_ID"]);
            if($iblockCode == "Forms_callback"){
                $eventType = "NEW_CALLBACK";
            }
            elseif($iblockCode == "Forms_Contact"){
                $eventType = "NEW_CONTACTS";
            }
            else{
                $eventType = "NEW_ELEMENT";
            }
            CEvent::Send($eventType, "s1", $arEventFields);
            //Выдаем нотификацию
            $arResult["OK"] = "Y";
